...
class TestParser:
    ...
    def test_parser_no_spaces(self):
        my_parser = Parser()
        quote = my_parser.return_remove_spaces(" bonjour  tout  le monde  ")
        assert quote == "bonjour tout le monde"

    def test_parser_punctuation(self):
        my_parser = Parser()
        quote_to_test_2 = my_parser.return_no_punctuation("openclassrooms !!!")
        assert quote_to_test_2 == "openclassrooms"

    def test_parser_stopwords(self):
        my_parser = Parser()
        quote_to_test_2 = my_parser.return_parsed("Bonjour Papy, dis-moi où est Openclassrooms ?")
        assert quote_to_test_2 == "openclassrooms"
